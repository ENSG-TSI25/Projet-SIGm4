...
std::vector<VectorLayer *> DataManager::loadVector(const std::string &chemin)
{
     ...
     for (const auto& layerName : couches)
    {
        ...
        std::string timestampField = reader.detectTimestampField(layerName);
        bool hasTemporal = !timestampField.empty();
        layer->setTemporalData(hasTemporal, timestampField);

        std::cout << "Layer: " << layerName
                  << " | Temporal: " << (hasTemporal ? "YES" : "NO")
                  << " | Field: " << timestampField << std::endl;
        ...
        for (const auto &f : features)
        {
            ...
            layer->addFeatureAttributes(f.fid, f.attributes);
        }
